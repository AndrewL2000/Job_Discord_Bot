...
@client.command()
async def jobs(ctx=None, field=None, job_type=None, ordering=None):
    ...
    if len(args) == 1:
        ...
        for job_type_map in mapping['gradconnection']['job_type']:
            if job_type in job_type_map or job_type_map in job_type:
    ...
    if app_name_list:
        embed = make_embed("Job Listings")
        embed.set_footer(text=f"Jobs listed here are grabbed from gradconnection (Courtesy of Richie Kim)")
        msg = await channel.send(embed=embed)
        i = 0
        for j in range(len(app_name_list)):
            ...
            i = i + 1
            await msg.edit(embed=embed)
            if mod(i,5)==0 and i!=len(app_link_list):
                embed = make_embed("Job Listings")
                embed.set_footer(text=f"Jobs listed here are grabbed from gradconnection (Courtesy of Richie Kim)")
                msg = await channel.send(embed=embed)   
...
async def help(ctx):
    ...
    embed.add_field(name="?jobs <field>(Optional) <qualifcation>(Optional) <order>(Optional)", value="Job Listings  \ne.g. ?jobs civil grad closing",inline=False)
...
async def on_loop():
    ...
    embed = make_embed("Commands")
    embed.add_field(name="?wakeup <@User>", value="Helps get the user's attention",inline=False)
    embed.add_field(name="?jobs <field>(Optional) <qualifcation>(Optional) <order>(Optional)", value="Job Listings",inline=False)
    embed.add_field(name="?modcheck <field>(Optional)", value="fish",inline=False)
